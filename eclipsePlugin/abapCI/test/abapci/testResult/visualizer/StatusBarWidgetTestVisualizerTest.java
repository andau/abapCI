...
public class StatusBarWidgetTestVisualizerTest {
	...
	private final StatusBarWidget statusBarWidget = Mockito.mock(StatusBarWidget.class);
	private final IProject project = Mockito.mock(IProject.class);
	private final TestResult unitTestResult = Mockito.mock(TestResult.class);
	private final TestResult atcTestResult = Mockito.mock(TestResult.class);
	private final Color TEST_COLOR = new Color(Display.getCurrent(), new RGB(100, 100, 100));
	...
	public void testSetResultVisualizerOutput() {
		...
		final List<AbapPackageTestState> abapPackageTestStates = new ArrayList<AbapPackageTestState>();
		...
		Mockito.verify(statusBarWidget)
				.setText(TEST_GLOBAL_TESTSTATE + ";     " + resultVisualizerOutput.getInfoline());
		Mockito.verify(statusBarWidget).setBackgroundColor(TEST_COLOR);

		abapPackageTestStates.add(
				new AbapPackageTestState(project.getName(), project.getName(), null, unitTestResult, atcTestResult));
		resultVisualizerOutput.setAbapPackageTestStates(abapPackageTestStates);
